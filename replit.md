...
## Overview
WhatShouldICharge is an AI-assisted junk removal job estimator. It provides price ranges, cubic yard estimates, visible-item breakdowns, and job type classifications by analyzing customer-uploaded photos using Claude vision AI. The platform streamlines the estimation process for junk removal businesses and provides tools for team management, customer interaction, and business analytics. Key capabilities include user authentication, subscription management via Stripe, a marketing landing page, an admin dashboard, a team portal with PIN-based authentication, and automated PDF estimate generation and email delivery. Language throughout the app uses "AI-assisted" (not "AI-powered") and explicitly communicates limitations (AI can only estimate visible items, not items hidden in piles or behind furniture).
